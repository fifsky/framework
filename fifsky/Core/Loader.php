...
class Loader {
     ...
     * @throws Exception
     */
    public function library($class_name, $singleton = true) {
        static $instances = array();

        $class_name = "fifsky\\library\\" . strtolower(str_replace("/", "\\", $class_name));

        if (!isset($instances[$class_name]) || !$singleton) {
            if (class_exists($class_name)) {
                $class                  = new $class_name();
                if(!$singleton){
                    return $class;
                }else {
                    $instances[$class_name] = $class;
                }
            } else {
                throw new Exception('Library ' . $class_name . ' not found.', Exception::ERR_NOTFOUND_LIBRARY);
            }
        }
