...
class allowHTML {
	...
	protected $allowed_attr = array( 'align' );

	//list of properties to allow in the css 'style' attribute (use * to allow all) - only applies if the 'style' html attribute is whitelisted
	...
	protected function format_input($input) {
        ...
        $input = str_ireplace(array('&nbsp;','&laquo;','&copy;','&raquo;'),array('\n','%[laquo]%','%[raquo]%','%[copy]%'),$input);
	...
	protected function format_output($input) {
		...
		$input = str_replace(array("&amp; ", "&amp;", "%[" . $rand . "]%", "&#13;",'\n','%[laquo]%','%[raquo]%','%[copy]%'), array("%[" . $rand . "]%", "&", "&amp; ", "","&nbsp;",'&laquo;','&copy;','&raquo;'), $input);
		//add doctype?
		if(!$this->fragment) {
			$input = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">' . $input;
		}
		...
		do {
			...
			if($node->hasAttributes() && in_array($node->tagName, $this->allowed_tags)) {
				...
				foreach($node->attributes as $attr) {
                    ...
                    if($node->tagName === "a" && $attr->name === "target" && $node->hasAttribute('target')) {
                        $clean = $this->clean_text($attr->value);
                    }
...
/*
  include LIB_PATH . 'allowHTML.php';
     //set options
     $options = array(
         'allowed_tags' => array( 'div', 'p', 'span', 'br', 'ul', 'ol', 'li', 'a[href|title|target]', 'img[src|alt|title|width|height]', 'font[size|face|color]', 'b', 'strong', 'i', 'em', 'u', 's', 'strike', 'blockquote', 'cite', 'caption', 'code', 'h1', 'h2', 'h3', 'h4', 'h5', 'table', 'tr', 'td', 'th', 'thead', 'tbody', 'tfoot' ,'object[width|height|data|type]','param[value|name]','embed[width|height|quality|type|src|flashvars]'), //allowing <p><br /><a> html tags only
         'allowed_attr'=> array( "align", "style" ), //allowing 'align' and 'style' html arrtibutes only
         'allowed_css' => array( "background",'text-decoration','font-style','font-weight','background-color','color','font-size','font-family','margin-left','text-align'), //allowing 'background' css property only
         'unsafe_text'=>array( ':alert', 'behavior:', 'behaviour:', 'content:', 'data:', ':expression', 'include-source:', 'mocha:', 'moz-binding:', 'script:' ),
     );

     $method = "sanitize";

    $diary_content  = cdn_path_replace(mb_convert_encoding(allowHTML(trim_space($_POST['diary_content']),$options, $method),'GBK','UTF-8'));
