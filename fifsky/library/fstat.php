...
class FStat
{
    ...
    {
        $this->is_stat = config('fstat', 'is_stat', false);

        if ($this->is_stat) {
            $this->use_db    = config('fstat', 'use_db');
            $this->db_prefix = config('fstat', 'db_prefix');
            $this->redis     = library('redisfactory')->getInstance(config('fstat', 'use_redis'));
        }
    }

    public function setRedis($redis)
    {
        $this->redis = library('redisfactory')->getInstance($redis);
    }
        ...
        foreach ($stat_arr as $dbf => $stats) {
            ...
            foreach ($stats as $k => $v) {
                if ($this->isExist($k, 'v4', $dbf)) {
                    ...
                    if (!strlen($v['v3']) || !strlen($v['v2']) || !strlen($v['v1'])) {
                        continue;
                    }
